...
try {
  ...
  assert.match(html, /href="\/assets\/tv-frame\.webp"/u)
  assert.match(html, /https:\/\/infiniteaislop\.ai\/assets\/og-card\.webp/u)
  assert.match(html, /type="application\/rss\+xml"/u)
  ...
  const poster = await fetch(`${origin}/assets/tv-frame.webp`)
  assert.equal(poster.status, 200)
  assert.match(poster.headers.get('content-type') || '', /^image\/webp/u)
  const socialCard = await fetch(`${origin}/assets/og-card.webp`)
  assert.equal(socialCard.status, 200)
  assert.match(socialCard.headers.get('content-type') || '', /^image\/webp/u)

  const robots = await fetch(`${origin}/robots.txt`)
  assert.equal(robots.status, 200)
  assert.match(robots.headers.get('content-type') || '', /^text\/plain/u)
  const robotsText = await robots.text()
  assert.match(robotsText, /Sitemap: https:\/\/infiniteaislop\.ai\/sitemap\.xml/u)
  assert.match(robotsText, /Disallow: \/api\//u)
